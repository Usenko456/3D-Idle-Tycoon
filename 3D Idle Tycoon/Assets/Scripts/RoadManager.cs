...
public class RoadManager : MonoBehaviour
{
    ...
    {
        placementMode = false;
        foreach (var position in temporaryPlacementPositions)
        {

                bool built = EconomyManager.Instance.TryBuild(
                    EconomyManager.BuildingType.Road,
                    new Vector2(position.x, position.z),placementManager.isLoading);

                if (!built)
                {
                    Debug.LogWarning($"Не вдалося списати кошти за дорогу на {position}");
                }
